...
namespace Domain.Common
{
    public static class Guard
    {
        ...
        public static void AgainstDateTimeIsDefault<TException>(DateTime dateTime, string name = "value")
            where TException : BaseDomainException, new() 
        {
            if (dateTime != default)
            {
          ...
          where TException : BaseDomainException, new()
        {
            ...
            ThrowException<TException>($"{propertyName} must be of range between {min} and {max} bytes!");
        }
